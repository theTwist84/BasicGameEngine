...
/* const reminder
    int test = 1;
    const int* const_value = &test;
    int* const const_ptr = &test;
    int copy = *const_value;
*/
...
double monitor_refresh_rate = 50;
bool vsync = true;

double game_update_time() { return 0.00001; }
double game_render_time() { return 0.005; }
double game_display_time() { return 0.000001; }
double busy_time() { return .000001; }

double system_time = randrange(rng) * 10000;
double timing_fuzziness = 1.0 / 60.0 * 0.005;
...
void simulate_display()
{
    ...
    int current_vsync = (int)round(system_time * monitor_refresh_rate);
...
int main_loop_tester()
{
    double prev_frame_time = system_time;
    last_vsync = (int)round(system_time * monitor_refresh_rate);
